...
세션/로그인/회원가입/복구 이메일/설정/계정 삭제를 실제 HTTP 세션 기준으로 빠르게 확인하려면:

```powershell
.\scripts\dev\Invoke-LocalSessionSmoke.ps1 -BaseUrl http://localhost:18080
```

- 이 스크립트는 실행 중인 `test` 프로필 앱을 기준으로 익명 보호 경로 리다이렉트, `d / d` 로그인, 회원가입 자동 로그인, signup recovery email confirm, 표시 이름 변경, 비밀번호 변경, 계정 삭제 요청 후 로그아웃까지 한 번에 확인합니다.
- 결과 리포트는 `build/local-smoke/reports/` 아래에 생성됩니다.
...
- `/app`의 오늘/이번 달 기준, 미래 날짜 차단, 날짜 표시, 스니펫 상대 시간은 브라우저 로컬 시간대가 아니라 서비스 기준 시간대인 `Asia/Seoul`로 계산합니다.
