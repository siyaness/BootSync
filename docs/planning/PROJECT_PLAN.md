...
- 운영 SMTP 제공자 기준 실메일 스모크 테스트와 confirm 증적 보고서가 추가됐다.
...
1. AWS 실제 배포 시작 (`ECR -> RDS -> EC2/k3s`)
2. 운영 AWS 자격증명 기준 S3 업로드 1회와 일일 스케줄 배치
3. prod-like 복원 리허설 수행과 `RTO 8시간` 실측
4. 정책 문서 초안 확정과 실제 공개값(운영 주체/문의처/도메인) 반영
...
2. operations evidence (`purge first scheduled run`, `S3 upload + daily schedule`, `prod-like restore + RTO`)
