...
- 실제 AWS 자격증명으로 S3 업로드가 성공하는지와 운영 스케줄 배치는 아직 남아 있다.
- `RTO 8시간`은 prod-like 복원 기준으로 별도 실측이 필요하다.

우선순위:

1. 운영 SMTP 설정과 실메일 점검
2. AWS 실제 배포 시작 (`ECR -> RDS -> EC2/k3s`)
3. S3 업로드 연결과 일일 스케줄 배치
4. prod-like 복원 리허설과 `RTO 8시간` 검증
...
2. operations evidence (`SMTP`, `S3`, `prod-like restore`, `purge first scheduled run`)
